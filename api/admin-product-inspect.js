...
module.exports=async function(req,res){
  ...
  try{
    ...
    const blocked=/HTTP\s+(401|403|429|503)|Sumber mengembalikan HTTP\s+(401|403|429|503)/i.test(String(e.message||''));
    const isShopee=/(^|\.)shopee\.|(^|\.)shp\.ee$/i.test(u.hostname||'');
    return res.status(200).json({ok:true,source:u.hostname,fallback:true,needsUserResolve:Boolean(blocked&&isShopee),actionUrl:u.toString(),name:title,slug:title,brand:'',category,summary:g.summary,pros:g.pros,cons:g.cons,specs:g.specs,price:'',oldPrice:'',rating:'',reviews:'',image:'',videoUrl:'',dataSource:'Tidak terverifikasi',sourceVerified:false,verificationLevel:'unverified',checkedAt,finalUrl:u.toString(),sourceSignals,redirectChain:[],notice:blocked&&isShopee?'Shopee menolak pembacaan server pada link ini. Sistem tidak melewati CAPTCHA/anti-bot dan tidak mengarang data. Buka link di browser/aplikasi Shopee, salin URL produk HTTPS yang tampil, lalu masukkan URL tersebut untuk pemeriksaan ulang.':`Halaman marketplace tidak bisa dibaca otomatis: ${e.message}. Jangan anggap harga kosong sebagai harga pasti; periksa halaman toko.`});
  }
};

...
$('#publish').onclick=async()=>{if(!token)token='session';if(!products.length)return setStatus('Katalog kosong.',false);const blocked=products.filter(p=>{const factKeys=['name','slug','category','brand','price','oldPrice','rating','reviews','unitsSold','shopName','stock','commissionRate','commissionAmount','priceMin','priceMax','oldPriceMin','oldPriceMax','marketplace','productId','detailLink','affiliateUrl','image','dataSource','sourceMethod','sourceCheckedAt'];const factsOk=factKeys.every(k=>String(p[k]??'').trim()!=='');const contentOk=String(p.summary||'').trim()&&String(p.seoTitle||'').trim()&&String(p.metaDescription||'').trim()&&String(p.caption||'').trim()&&Array.isArray(p.pros)&&p.pros.length&&Array.isArray(p.cons)&&p.cons.length&&Array.isArray(p.faq)&&p.faq.length&&p.specs&&Object.keys(p.specs).length;return !factsOk||!String(p.name||'').trim()||!String(p.category||'').trim()||String(p.category).toLowerCase()==='belum ditentukan'||String(p.name).toLowerCase()==='produk belum teridentifikasi'||!contentOk||(p.verificationRequired===true&&p.sourceVerified!==true)});if(blocked.length)return setStatus(`${blocked.length} produk belum lengkap. Semua kolom fakta, sumber, media, konten, SEO, spesifikasi, FAQ, dan caption wajib terisi sebelum publish. Jika fakta tidak tercantum pada sumber, gunakan keterangan 'Tidak tercantum pada sumber yang diverifikasi.'`,false);setStatus('Menyimpan ke GitHub...');try{const r=await fetch('/api/admin-products',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({products})});const d=await r.json();if(!r.ok)throw Error(d.error||'Gagal menyimpan');setStatus(`Berhasil dipublish: ${d.products} produk (${d.active} aktif). Commit ${d.commit||'-'}. Vercel akan membuat deployment baru jika repository terhubung.`)}catch(e){setStatus(e.message,false)}};
